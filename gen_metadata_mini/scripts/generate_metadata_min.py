...
def get_fish_metadata (insts, im):
    ...
    dict_fish={"fish_num" : 0, "bbox":"None", "mask_type":"None", "eye_bbox":"None"}
    
    # Select the fish with highest score
    main_fish_inst, num_fish = select_main_fish(insts)
    _, im_gray = ut.enhance_contrast(im)
    ...
    if num_fish>=1:
        ...
        dict_fish ['bbox'] = bbox
        if analysis_failed: 
            dict_fish ['mask'] = 'detectron mask'
        else:     
            dict_fish ['mask'] = 'pixel_analysis_mask'
...
def find_main_eye(Boxes_fish, insts):
    ...
    for idx in range(len(eyes_insts)):
        ...
        # find the first eye (highest score) in the fish (overlap>0.95)
        if overlap_fish_eye > 0.95:            
