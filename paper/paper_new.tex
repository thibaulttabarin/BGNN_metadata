...
\begin{abstract}
Metadata is a key data source particularly for researchers seeking use apply achine learning to the vast collections of digitized specimens. Unfortunately, the metadata available is often sparse and, at times, erroneous; and it is prohibitively expensive to address these limitations through traditional, manual means. This paper reports on a research applying machine-driven approaches to analyze digitized fish images and extract a variety of important features. The digitized fish specimen are being analyzed as part of Biology Guided Neural Networks (BGNN) initiative, where researchers are developing a novel class of artificial neural networks using phylogenies and anatomy ontologies. The automatically generated metadata is crucial for identifying high-quality images for the neural network to support predictive analyses.%predictive analytics?
Research combining machine learning  and  image  informatics techniques allow us to rapidly enrich existing metadata associated for a sample of 7244 images from the Illinois Natural History Survey (INHS). The sample also includes 22 manually generated image-quality metadata properties, aiding our comparison.  Results show we can  accurately generate metadata for six key %6? 
metadata properties: fish quantity, fish orientation, image scaling based on ruler identification and measurement, and general image quality metrics. Results also show can also x, y, z.
...
The automatic process out-performs humans in terms time and labor cost, and provides a novel solution for leveraging digitized specimens for machine learning. This research has implications for harnessing the tens of thousands of digitized specimens stored in open access repositories found wold-wide.
...
Technical metadata, such as camera settings and temporal information (date and time) are automatically generated during a digitization sequence, following standards such as Exchangeable image file format (Exif)~\cite{Exif}. The camera's technical metadata that is automatically captured and inserted into digital image files at the time of capture. Some of this metadata may be useful when selecting a machine learning sample. A researcher may desire images with specific properties, such as being captured chiefly with a certain aperture setting. Even so, the majority of automatically captured technical metadata associated with digitized specimens is also insufficient for computational research, leaving researchers to rely on manually generated descriptive metadata, which itself is sparse and prone to human error. Fish image analysis research, as reviewed below, provides an opportunity to address current metadata shortcomings and needs specific to the selection of high-quality digitized specimen images for neural networks and the application of machine learning.
...
Digitized specimen accessible in open access repositories provide a rich, extensive data source machine-learning and scientific discovery, however, these resources remain largely untapped due image quality and metadata limitations. The overall goal is addressing this need by combining machine-learning and image informatics techniques. The overall goal of our research is to offer a machine-driven [*novel/innovative?? blah...] approach to current manual metadata generation that is prohibitively costly in both terms of labor and time, and to further offer a [*novel] approach to computing higher-level metadata that supports scientific inquiry. Three key objectives are to:

GOAL 1) Test [*Explore, Demonstrate the us of] Facebook's AI Research's dectectron tool
Specific aims are to use dectectron to identify five object classes: fish, fish eyes, rulers, and the numbers 2 and 3 on rulers, as shown in Figure 

GOAL 2) Pursue [*Investigate] image processing at the pixel level.
Pilot testing found the dectectron bounding boxes could be more tightly enclosed the detected objects. We will see if additional masking background as part of the fish, or the
bounding box may clip away part(s) of the fish with pixel analysis methods commonly
found in image informatics. Specific aim [*objectives] of our research are to:
...
1. Identify the appropriate threshold. We will compute optimal scaling between [*or is it Otsu thresholding] to identify an improved threshold value. [Or we will determine is we can computed as the halfway point between the Otsu threshold value and the mean of the background intensities.]

2. Consolidate the foreground

3. Adjusting the bounding box. [We will use computational methods for masking [calcuate a mask?] to determine if bounding box needs to be shrunk or expanded]
% Jane confused here, if this belongs here, or below

4. Developing a fallback approach for when flood fill operations behave unexpectedly the image is too washed out.

GOAL 3) Compute [*Test our ability to calculate] the following set of higher-level metadata properties: 1) contrast (intensities of the foreground and background, 2) centroid and eye center, and if eye is found, the fish 3) side 4) primary axis and clock value, finally the 5) scale and length.
...
The automated metadata generation methods for our project were developed to work on a specific set of images, the INHS\ Fish Collection~\cite{INHS}.  Most of these images have been configured, produced and acquired with a standard procedure.  The images used for our study contain one fish placed on a bright, white background and contain an information tag and the same ruler.  See Figure \ref{fig:teaser} for two example images from the collection. While training and focusing our system on images with very similar compositions and visual properties may limit its immediate applicability, our efforts do demonstrate the potential that machine learning and image informatics techniques have for automatically generating metadata for biological specimen image collections.
...
\begin{table*}[t]
    \centering
    \caption{Metadata properties (* indicates higher order properties)}
