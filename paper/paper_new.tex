...
\section{Conclusions}
%jane Merge this type of thing with other - Overall our results show a proof of concept and offer a path forward for using object detection technology, enhanced by image informatics techniques, particulalry pixel analysis to improve and enrich metadata that enables researchers to apply machine learning to investigate scientific research questions on a scale that has never been pursued before. Our investigation focused on fish as a , but there is more work to be done to examine the biodiversity among specific types of species. It's important to keep in mind that "fish" are vertebrate animals (phylum Chordata), and there are over 34,000 species, not to mention the many unidentified species. Species names are labels, and the discovery of species variation depends on genotype and phenotype. The ability to analyze thousands of images of a single fish species computationally can lead to new discoveries that are impossible to manually pursue. The immediate impact of our research is being integrated into the BGNN workflow that will enable both large-scale and individual Digital libraries that have fish digitized fish specimen images on a much larger scale, and XXXX.

%next paragrah - The implication of our work extends well beyond digitized fish images. The research presented is grounded in object detection and the pixel analysis along with computation enables the generation of metadata properties that are crucial to research and analysis across a wide expanse of disciplines. 
