...
The \verb|primary_axis| of a fish can be calculated by taking the covariance\todo{I could explain this in more detail if necessary. (DEB No, that's OK.} of its blob in \(x\) and \(y\), which yields its principle eigenvector.
...
% \todo{Not sure if I can phrase it that way or not since I haven't looked at all the images to see if the ruler itself was found or something else was labeled a ruler}
...
a 96.5\% correct rate.
\todo{Comment on the 6 cases. What failed?}
...
computation is 96.4\%.
\todo{Comment on the cases where we were unable to compute clock value. What failed?}
