...
%do we add grant now, or wait til acceptance. i think we need to ultimately remove any identifying info., e.g., bgnn, and even mention of metadata research center. 
...
This limitation is magnified, when trying get a handle on the actual quality of the digitized specimen. Descriptive oriented standards support search and retrieval, but metadata properties used to record image quality is severely limited and generally missing. This point is addressed in detail in \cite{leipzig2021biodiversity}, justifying the capture of 22 metadata properties supporting selection of high quality images for the development of a bio-generated neural network.
...
Metadata researchers are aware of challenges posted by manual metadata generation, both in terms of time,  //Technical metadata capturing image lighting, camera, date of digitization and other information are automatically generated, but 
...
\textit{Like I said in slack, the standard deviation of background does not currently correlate with the is\_background\_uniform property. I still have to look into the dark normal bright calculations in the new way we discussed.}
...
\begin{itemize}
    ...
    \item Check for color channel inequality when masking fish
\end{itemize}
\section{Conclusion}
\textbf{Todo}
