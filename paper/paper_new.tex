...
%do we add grant now, or wait til acceptance. i think we need to ultimately remove any identifying info., i think bgnn is likely okay; but, perhaps mention metadata research center (MRC) need to be removed?. if so, i kind of like to put it back in for final version, if accepted. it helps with getting funding ... you are both listed on the MRC page. we should check on the identifying info. removed, or not.
%for the record: Research supported by NSF OAC #1940233 and #1940322.
...
\begin{abstract}
Metadata is a key data source particularly for researchers seeking use [apply] machine learning to the vast collections of digitized specimens. Unfortunately, the metadata available is often sparse and, at times, erroneous; and it is prohibitively expensive to address these limitations through traditional, manual [*human] means. This paper reports on a research applying machine-driven [*automatic] approaches to analyze digitized fish images and extract a variety of important features. The digitized fish specimen are being analyzed as part of the XXX project [*Biology Guided Neural Networks (BGNN)], where researchers are developing a novel class of artificial neural networks using phylogenies and anatomy ontologies. The paper reports on research combining machine  learning  and  image  informatics  tools and techniques. A sample 7247 images from the INHS (write-out) were examined via automatic processes; and results show we can  accurately determine metadata, such as, fish quantity, fish orientation, image scaling based on ruler identification and measurement, and general image quality metrics. Results also indicate that automatic approaches "can outperform humans" [and/or "expedite"] the creation of metadata that's critical to leveraging digitized specimens for machine learning. The sections that follow cover background context, reviews the research design, reports results Next,  the  results  are presented, followed by a discussion. 
%items in square brackets are just other suggested words, e.g., select one or the other, or .... 
%something about implications above
%something about comparison to human results.
...
%\cite{heidorn2008automatic} speciment labels \cite{manso2004automatic} cartographic info.
%above paragraph..need to finesse a couple of sentences w/results, weave in these 2 citations, and then say we can look to image analysis.  
...
%I think we can go from the above, statement, directly to the paragraph that starts with "Given the many..." Then, specifically state the goal of this paper is to XXXX address the need, or the goal is to investigate, test, (pick your verb) an approach automatically generating metadata, combining off-the-shelf software and custom written, or is it simply test off-the-shelf software, and later the realization that customization happens.
...
For 25 of the images, 2 fish were detected, 3 fish were detected for 3 images, and for 7 images no fish were found. In the case of greater than 1 fish, 9 of the 28 contained tags which overlapped the fish and were themselves labeled as a second fish. Of the remaining 17, \verb|detectron| erroneously labeled the fish as two separate fish objects, or labeled a subsection of the fish a second time. The 7 fish that were not detected were quite small, which is something that is a type of specimen that is currently lacking from the training set. See the left image in Figure~\ref{fig:fish_detect_wrong} for an example of this. Fish that were double labeled were generally quite large and/or dissimilar from the fish found in the training set, such as the right image in Figure~\ref{fig:fish_detect_wrong}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.49\linewidth]{images/none1}
  \includegraphics[width=0.49\linewidth]{images/double1}
  \caption{A fish that was not detected (left) and a fish that was detected twice (right).}
...
The contrast of an image is an important aspect of the image quality for the BGNN study. Ideally, for INHS images the fish should be well lit and vibrant, and the background should also be as light and white as possible. Figure~\ref{fig:histo} shows the distribution of the difference between the mean intensity of the foreground (i.e.\ the pixels of the fish) and the mean intensity of the background within the fish's bounding box. The overall mean of the distribution is \(144.26\), with a standard deviation of \(15.79\). Images on the low end of this distribution exhibit poor foreground--background contrast, and images on the high end likely contain poorly lit specimens. Examples of each type of image (low, normal, and high contrast) can be seen in Figure~\ref{fig:contrast}. The top left image has a mean intensity of 103 (low), the top right 144 (normal), and the bottom 186 (high).
...
Specimen brightness is one of the 22 hand recorded properties. It is encoded as \verb|dark|, \verb|normal| or \verb|bright|. These values correspond to the measurement of mean foreground intensity by the automated system. The correlation between the two measurements can be seen in Table~\ref{tab:bright} below.
\begin{table}[H]
    \centering
    \caption{Brightness measurements}
    \label{tab:bright}
    \begin{tabular}{cccc}
        \toprule
        \textbf{Class} & \textbf{Mean Intensity} & \textbf{Standard Deviation} & \textbf{Instances}\\
        \midrule
        Dark & 75.15 & 13.49 & 1800\\
        Normal & 93.64 & 14.78 & 5186\\
        Bright & 108.39 & 15.21 & 242\\
        \bottomrule
    \end{tabular}
\end{table}

\textbf{Todo}: I suppose more should be said here.
